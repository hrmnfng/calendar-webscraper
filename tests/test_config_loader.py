...
Uses tmp_path (pytest built-in fixture) to write real YAML files to a temp
directory, keeping tests hermetic without touching the real calendar-configs/.
"""
...
class TestCalendarConfig:
    ...
    def test_valid_config_created(self):
        cfg = CalendarConfig(name="My Cal", url="https://example.com", color_id=3)
        assert cfg.name == "My Cal"
        assert cfg.url == "https://example.com"
        assert cfg.color_id == 3
...
# ---------------------------------------------------------------------------
# load_configs — active/disabled file filtering
# ---------------------------------------------------------------------------
...
class TestLoadConfigsFileFiltering:
    ...
    def test_loads_multiple_active_configs(self, tmp_path):
        for i in range(3):
            _write_yaml(
                str(tmp_path),
                f"config-team-{i}.yaml",
                {"name": f"Team {i}", "url": f"https://example.com/{i}", "color_id": i + 1},
            )
        configs = load_configs(str(tmp_path))
        assert len(configs) == 3

    def test_returns_configs_in_sorted_filename_order(self, tmp_path):
...
# ---------------------------------------------------------------------------
# load_configs — malformed files are skipped, not fatal
# ---------------------------------------------------------------------------

class TestLoadConfigsMalformedFiles:

    def test_skips_file_with_invalid_color_id(self, tmp_path):
        bad = {"name": "Bad", "url": "https://example.com", "color_id": 99}
        _write_yaml(str(tmp_path), "config-bad.yaml", bad)
    ...
    def test_skips_file_missing_name(self, tmp_path):
        bad = {"url": "https://example.com", "color_id": 3}
        _write_yaml(str(tmp_path), "config-bad.yaml", bad)
        _write_yaml(str(tmp_path), "config-good.yaml", VALID_CONFIG)
        configs = load_configs(str(tmp_path))
        assert len(configs) == 1
    ...
    def test_all_bad_files_raises_system_exit(self, tmp_path):
        bad = {"url": "https://example.com", "color_id": 99}  # missing name, bad color
        _write_yaml(str(tmp_path), "config-bad.yaml", bad)
        with pytest.raises(SystemExit):
            load_configs(str(tmp_path))
