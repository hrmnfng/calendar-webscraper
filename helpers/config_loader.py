...
Active configs must be named `config-*.yaml`. Files ending in `.yaml.disable`
are silently skipped, making it easy to toggle calendars without deleting files.
"""
...
class CalendarConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.color_id, int) or not (1 <= self.color_id <= 11):
            raise ValueError(
                f"Calendar config 'color_id' must be an integer between 1 and 11, got {self.color_id!r}"
            )
...
def load_configs(config_dir: str) -> list[CalendarConfig]:
    ...
    A file is considered active when its name matches ``config-*.yaml``
    (files ending in ``.yaml.disable`` are ignored).
...
def _iter_configs(config_dir: str) -> Iterator[CalendarConfig]:
    ...
    for filename in sorted(os.listdir(config_dir)):
        ...
        if not isinstance(raw, dict):
            logger.warning(f"Skipping {filename}: expected a YAML mapping, got {type(raw).__name__}")
            continue
