...
The script loads a `.env` file from the project root automatically if one is present (values already set in the shell environment take precedence). Copy the block below into a `.env` file and fill in your values:
